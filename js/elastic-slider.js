...
ElasticSlider.prototype = {
  init: function (options) {
    ...
    this.maxStretch =
      options && options.hasOwnProperty('maxStretch')
        ? parseFloat(options.maxStretch)
        : 100
    this.bezierLen =
      options && options.hasOwnProperty('bezierLen')
        ? parseFloat(options.bezierLen)
        : 80

    this.rect = this.el.getBoundingClientRect()
    console.log('this.rect', this.rect)
    // this.rect.x = 500
    this.sliderWidth = this.rect.width / 2
  ...
  initPaths: function () {
    this.optionsLeft = {
      ...
      width: 0,
      bezierLen: this.bezierLen,
      offset: this.sliderWidth,
      leftSide: true,
    }
    this.optionsLeftAux = {
      x1: 0,
  ...
  createPath: function (o) {
    ...
    o.leftSide
      ? out.push(x1, y2, 'L', x1, y1)
      : out.push(x1 + offset * 2, y2, 'L', x1 + offset * 2, y1)
    return out.join(' ')
  },
  ...
  initMove: function () {
    ...
    self.el.ontouchmove = function (e) {
      animMove(e.touches[0].clientX)
    } // touchmove event
    function animMove(z) {
      if (self.mouseDown && !self.animating) {
        self.width = z - self.initialX
        console.log('z', z, 'self.initialX', self.initialX)
        if (z > self.initialX) {
          self.prevAnimation()
        } else {
          self.nextAnimation()
  ...
  prevAnimation: function (go) {
    ...
    if (self.width < self.maxStretch) {
      ...
      self.pLeft
        .stop()
        .animate(
          { transform: 't-' + (self.maxStretch * 3 - 50) + ',0' },
          300,
          mina.easein,
          function () {
            self.pLeft.animate(
              { transform: 't-' + self.maxStretch * 3 + ',0' },
              200,
              mina.easein
            )
          }
          ...
          function () {
            self.pLeft.animate(
              ...
              function () {
                self.prev()
                self.updateStates()
                self.pLeft.stop().attr({
                  d: self.curveLeft,
                  transform: 't-' + self.sliderWidth + ', 0',
                })
  ...
  nextAnimation: function () {
    ...
    if (self.width > -self.maxStretch) {
      ...
      self.pRight.stop().animate(
        {
          transform:
            't-' + (self.sliderWidth * 2 - (self.maxStretch * 3 - 50)) + ',0',
        },
        300,
        mina.easein,
        function () {
          self.pRight.animate(
            {
              transform:
                't-' + (self.sliderWidth * 2 - self.maxStretch * 3) + ',0',
            },
            200,
            mina.easein
          )
      ...
      self.pRight.animate(
        { path: middleCurveRight },
        200,
        mina.easein,
        function () {
          self.pRight.animate(
            { path: middleCurveRight2 },
            100,
            mina.easein,
            function () {
              self.pRight.animate(
                { path: totalCurveRight },
                200,
                mina.easein,
                function () {
                  self.next()
                  self.updateStates()
                  self.pRight.stop().attr({
                    d: self.curveRight,
                    transform: 't-' + self.sliderWidth + ', 0',
                  })
                  self.animating = false
  ...
  changing: function () {
    const widgetEvent = new CustomEvent('myevent', {
      bubbles: true,
      detail: {
        w: this,
        indexCurrentSlide:
          this.getNextIndex() - 1 === -1 ? 2 : this.getNextIndex() - 1,
      },
    })
